    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        {DC_SURVEY_QUESTIONS.map(({ key, label, question, placeholder }) => (
          <div key={key} className="bg-[#0d1b33] rounded-lg p-5 sm:p-6">
            <label className="block text-xs font-bold text-[#4a9eff] uppercase tracking-wider mb-1">
              {label} *
            </label>
            ...
            <textarea
              ...
              rows={2}
              className={`w-full px-4 py-3 bg-[#0a1628] border rounded-xl text-white placeholder-slate-600 focus:outline-none focus:border-[#4a9eff] focus:ring-2 focus:ring-[#4a9eff]/20 transition-all resize-none ${
                survey[key]?.trim() ? 'border-[#4a9eff]/30' : 'border-slate-700/30'
              }`}
            />
          </div>
        ))}
