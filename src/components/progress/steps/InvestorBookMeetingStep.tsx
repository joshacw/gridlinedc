...
export default function InvestorBookMeetingStep({ enquiryId }: Props) {
  ...
  const handleConfirm = async () => {
    setConfirming(true);
    try {
      await advanceStep({ enquiryId, stepKey: "investorExploreMeeting" });
    } catch (error) {
    ...
    <div className="space-y-6">
      ...
      <div className="bg-[#0d1b33] rounded-lg p-6 sm:p-8">
        <div className="flex items-start gap-4 mb-6">
          ...
          <div>
            <h2 className="text-xl sm:text-2xl font-bold text-white">Explore the Opportunity</h2>
            <p className="text-slate-400 mt-1 text-sm sm:text-base">
              Schedule a call with our investment team to learn more about the GRIDLINE opportunity.
            </p>
          </div>
        </div>
        ...
        <div className="bg-slate-800/30 rounded-lg p-5 mb-6">
          ...
          <ul className="space-y-2">
            {[
              'A 30-minute introductory call with a member of our investment team',
              'Overview of the GRIDLINE data centre aggregation strategy',
              'Discussion of investment structures, timelines, and returns',
            ].map((item, i) => (
        ...
        {/* Calendar embed */}
        <iframe
          src={GHL_INVESTOR_CALENDAR_URL}
          style={{ width: '100%', border: 'none', overflow: 'hidden' }}
          scrolling="no"
          className="min-h-[600px] sm:min-h-[700px] rounded-lg"
          title="Schedule an Investment Call"
        />
      </div>
