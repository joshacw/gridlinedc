...
import { INVESTOR_SURVEY_QUESTIONS } from '@/constants';
...
export default function InvestorSurveyStep({ enquiryId, existingSurvey }: Props) {
  const submitSurvey = useAction(api.progress.submitInvestorSurveyFromProgress);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const advanceTimer = useRef<ReturnType<typeof setTimeout> | null>(null);

  const [survey, setSurvey] = useState<SurveyData>(() => {
    const initial: SurveyData = {};
    INVESTOR_SURVEY_QUESTIONS.forEach(q => {
  ...
  const [currentIndex, setCurrentIndex] = useState<number>(() => {
    ...
    const firstUnanswered = INVESTOR_SURVEY_QUESTIONS.findIndex(
      q => !existingSurvey[q.key]?.trim()
    );
    return firstUnanswered === -1 ? INVESTOR_SURVEY_QUESTIONS.length - 1 : firstUnanswered;
  });

  const currentQuestion = INVESTOR_SURVEY_QUESTIONS[currentIndex];
  const currentAnswered = survey[currentQuestion.key]?.trim().length > 0;
  const isLastQuestion = currentIndex === INVESTOR_SURVEY_QUESTIONS.length - 1;
  ...
  const handleAnswer = useCallback((key: string, value: string) => {
    ...
    const q = INVESTOR_SURVEY_QUESTIONS[currentIndex];
    if (q.type === 'choice' && currentIndex < INVESTOR_SURVEY_QUESTIONS.length - 1) {
  ...
  const handleNext = () => {
    if (currentIndex < INVESTOR_SURVEY_QUESTIONS.length - 1) {
      setCurrentIndex(prev => prev + 1);
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      await submitSurvey({
        enquiryId,
        investorSurvey: {
          investorType: survey.investorType || undefined,
          accreditationStatus: survey.accreditationStatus || undefined,
          investmentRange: survey.investmentRange || undefined,
          investmentTimeline: survey.investmentTimeline || undefined,
          geographicPreference: survey.geographicPreference || undefined,
          priorDCExperience: survey.priorDCExperience || undefined,
        },
      });
    } catch (error) {
      console.error('Error submitting investor survey:', error);
    ...
    <div className="space-y-6">
      ...
      <div className="bg-[#0d1b33] rounded-lg p-6 sm:p-8">
        <div className="flex items-start gap-4 mb-4">
          ...
          <div>
            <h2 className="text-xl sm:text-2xl font-bold text-white">Request Offer Detail</h2>
            <p className="text-slate-400 mt-1 text-sm sm:text-base">
              Answer {INVESTOR_SURVEY_QUESTIONS.length} quick questions so we can tailor the investment details to you.
            </p>
          </div>
        </div>
        ...
        <WizardProgress
          ...
          questions={INVESTOR_SURVEY_QUESTIONS}
          answers={survey}
        />
      </div>
      ...
      <div className="flex items-center justify-between gap-4">
        ...
        >
          {isSubmitting ? 'Submitting...' : isLastQuestion ? 'Submit Details' : 'Next'}
        </button>
      </div>
    </div>
  );
